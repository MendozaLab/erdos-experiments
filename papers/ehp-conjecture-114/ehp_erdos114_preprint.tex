...
\title{Computational Verification of the Erd\H{o}s--Herzog--Piranian\\Conjecture for Degrees $3 \leq n \leq 10$}
...
\date{March 2026\\[6pt]\textsc{Draft for Review --- Not for Distribution}}
...
\item Computational verification of the EHP conjecture for all $n \in \{3, 4, 5, 6, 7, 8, 9, 10\}$ via certified branch-and-bound with IEEE~1788 interval arithmetic (\S\ref{sec:results}).
\item A new closed-form expression for $L(z^n - 1)$ valid for all~$n$ (\S\ref{sec:formula}):
\[
L(z^n - 1) = 2^{1+1/n} \cdot \sqrt{\pi} \cdot \frac{\Gamma\bigl(1/(2n)\bigr)}{\Gamma\bigl(1/(2n) + 1/2\bigr)}.
\]
...
\item Analysis of the monotonic margin growth from 17.1\% to 71.4\% and its implications for the gap between $n = 10$ and Tao's threshold (\S\ref{sec:gap}).
\end{enumerate}
...
\begin{table}[ht]
\centering
\caption{Computational verification of the EHP conjecture for $n \in \{3, \ldots, 10\}$. All intervals are certified under IEEE~1788 rounding via the \texttt{inari} crate. Dim.\ is the reduced parameter space dimension $2n-5$.}
\label{tab:results}
\small
\begin{tabular}{crccc}
\toprule
...
The margin increases monotonically from 17.1\% at $n=3$ to 71.4\% at $n=10$ (Table~\ref{tab:margins}). This trend is consistent with Tao's asymptotic result and suggests that the separation between the optimal polynomial and its competitors grows with degree.
...
\subsection{The gap between $n = 8$ and Tao's threshold}
...
The initial box count grows as $2^{2n-5}$, but the growing margins allow all non-extremizer boxes to be eliminated at level~0 for $n \geq 5$:
\begin{itemize}[nosep]
\item $n = 8$: 11 dimensions, 2{,}048 boxes, 1\,s.
\item $n = 9$: 13 dimensions, 8{,}192 boxes, 4\,s.
\item $n = 10$: 15 dimensions, 32{,}768 boxes, 12\,s.
\item $n = 12$: 19 dimensions, estimated $\sim$20 minutes.
\item $n = 15$: 25 dimensions, estimated $\sim$1 day.
\end{itemize}
...
The conjunction of Tao's asymptotic result (all $n \geq N_0$) and our computational verification ($n \in \{3, \ldots, 10\}$) provides strong evidence for the full EHP conjecture:
...
\item \textbf{Gap $n \in \{11, \ldots, N_0 - 1\}$}: Open, subject to future investigation.
\end{enumerate}
