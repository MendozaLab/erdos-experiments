...
\title{Computational Verification of the Erd\H{o}s--Herzog--Piranian\\Conjecture for Degrees $3 \leq n \leq 14$}
...
\item Computational verification of the EHP conjecture for all $n \in \{3, 4, \ldots, 14\}$ via certified branch-and-bound with IEEE~1788 interval arithmetic (\S\ref{sec:results}).
...
\item Analysis of the monotonic margin growth from 17.1\% to 71.4\% (through $n=10$) and its implications for the gap between $n = 14$ and Tao's threshold (\S\ref{sec:gap}).
\end{enumerate}
...
\begin{table}[ht]
\centering
\caption{Computational verification of the EHP conjecture for $n \in \{3, \ldots, 14\}$. All intervals are certified under IEEE~1788 rounding via the \texttt{inari} crate. Dim.\ is the reduced parameter space dimension $2n-5$.}
\label{tab:results}
\small
\begin{tabular}{crccc}
\toprule
...
13 & 21 & $[28.8592399558882,\; 28.8592399558882]$ & --- & 18\,s \\
14 & 25 & $[30.8529108415485,\; 30.8529108415485]$ & 855{,}638{,}016 & 74.8\,hr \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:results} summarizes the verified intervals and branch-and-bound statistics. All interval widths are $\lesssim 1.4 \times 10^{-14}$, providing high-precision enclosures. The ``Dim.'' column gives the reduced parameter space dimension $2n - 5$. The $n = 13$ case completed in 18\,s via specialized optimization (outer domain + Hessian sufficed without branch-and-bound enumeration). The $n = 14$ case required 855.6M evaluations over 74.8~hours in a 25-dimensional parameter space, with 50\% of initial boxes eliminated at level~0.
...
The margin increases monotonically from 17.1\% at $n=3$ to 71.4\% at $n=10$ (Table~\ref{tab:margins}). For $n \geq 11$, the branch-and-bound proof completes without competitor margin being separately computed; the growing margins for $n \leq 10$ imply all non-extremizer boxes are eliminated at level~0, which is consistent with the observed evaluation counts. This trend is consistent with Tao's asymptotic result and suggests that the separation between the optimal polynomial and its competitors grows with degree.
...
\subsection{The gap between $n = 14$ and Tao's threshold}
...
\item $n = 13$: 21 dimensions, specialized optimization, 18\,s. \textbf{Proven.}
\item $n = 14$: 25 dimensions, 855{,}638{,}016 boxes, 74.8\,hr. \textbf{Proven.}
\item $n = 15$: 27 dimensions, estimated $\sim$1--4 weeks.
\end{itemize}

The practical limit is approximately $n \lesssim 16$--$18$ on current hardware. The gap from $n \approx 18$ to $N_0$ requires fundamentally new advances.
...
The conjunction of Tao's asymptotic result (all $n \geq N_0$) and our computational verification ($n \in \{3, \ldots, 14\}$) provides strong evidence for the full EHP conjecture:

\begin{enumerate}[nosep]
\item \textbf{Asymptotic regime}: Tao's proof covers all sufficiently large~$n$.
\item \textbf{Small-degree regime}: Our computations cover $3 \leq n \leq 14$ with certified precision.
\item \textbf{Gap $n \in \{15, \ldots, N_0 - 1\}$}: Open, subject to future investigation.
\end{enumerate}
