...
%% ────────────────────────────────────────────────
%% [C1] Abstract: narrowed "Shannon entropy...incapable" to
%%   "all E_2-determined statistics are blind"; removed
%%   "forced spectral relaxation mechanism".
%% ────────────────────────────────────────────────
\begin{abstract}
...
This $S_2$ blindness barrier implies that all statistics determined
by the additive energy $E_2(A)$ alone --- including spectral variance,
spectral purity, and the entropy of the sum distribution --- are
...
before declining.  This trajectory is consistent with the Fourier
uniformity theorem of Ortega-Cerd\`a and Prendiville and may
be relevant to Erd\H{o}s Problem~\#30,
though we do not prove a spectral relaxation mechanism here.
\end{abstract}
...
%% ────────────────────────────────────────────────
%% [C2] Intro item (iii): "Spectral phase transition" demoted
%%   to "Non-monotone trajectory (Computational Observation)".
%% ────────────────────────────────────────────────
\item \textbf{Non-monotone spectral trajectory}
(\cref{obs:trajectory}; \emph{Computational Observation}):
...
shows precisely why any $E_2$-determined approach cannot distinguish
Sidon sets: the additive energy $E_2(A)$ is universal for Sidon sets
(\cref{lem:sidon-energy}), so all statistics that factor through
$E_2$ alone are blind.
...
%% ────────────────────────────────────────────────
%% [C3] Corollary part (d): explicit "sum-distribution
%%   entropy", E₂-determined qualifier; bold note that
%%   S₁ is NOT in the blind class.
%% ────────────────────────────────────────────────
...
\item The Shannon entropy of the \textbf{sum distribution}
$H(\{r_2(n)/k^2\}_n)$, since the distribution of values
$\{r_2(n)\}_n$ is fully determined by~$E_2$ for Sidon sets.
\end{enumerate}
\textbf{Note.} The von Neumann entropy $S_1(\rho_A)$ is
\emph{not} in the blind class: it depends on the full
eigenvalue distribution $\{\lambda_t\}_{t=0}^{N-1}$,
not just its second moment, and varies across Sidon
sets of the same size (\cref{sec:transition}).
\end{corollary}
...
%% ────────────────────────────────────────────────
%% [C4] Remark: "Shannon-level approach" → "E₂-determined
%%   approach" for internal consistency.
%% ────────────────────────────────────────────────
\begin{remark}
\cref{thm:S2-universal} explains why the $E_2$-determined entropic
approach of Goh~\cite{Goh2024} characterizes Sidon sets as extremals
of small entropic energy but cannot further \emph{distinguish among}
Sidon sets.  His framework factors through $E_2(A)$, and our theorem
shows that any statistic determined by $E_2$ alone is blind.
...
%% ────────────────────────────────────────────────
%% [C5] Section title: "Spectral phase transition" →
%%   "Non-monotone spectral trajectory: computational
%%   observations"; opener flags empirical status.
%% ────────────────────────────────────────────────
\section{Non-monotone spectral trajectory: computational observations}
\label{sec:transition}

\emph{The results in this section are empirical observations
from finite computation, not theorems.  They are included to
motivate the open questions in~\cref{sec:open} and to provide
reproducible data for future work.}
...
\begin{observation}[Non-monotone spectral trajectory]
\label{obs:trajectory}
...
%% ────────────────────────────────────────────────
%% [C6] Discussion §5.2: retitled "Spectral budget squeeze:
%%   a conjectural mechanism"; heuristic explicitly flagged
%%   as unproved.
%% ────────────────────────────────────────────────
\subsection{Spectral budget squeeze: a conjectural mechanism}

The non-monotone trajectory of $\lmax/k$ at fixed~$N$
(\cref{obs:trajectory}) \emph{suggests} a heuristic that we
state explicitly as unproved.

As~$k$ grows, two constraints tighten simultaneously:
...
BFR upper bound.  \emph{We do not prove such an improvement here.}

%% ────────────────────────────────────────────────
%% [C7] Discussion §5.3: "potential proof strategy" isolated
%%   in a \begin{conjecture} block; open gap explicitly named.
%% ────────────────────────────────────────────────
...
minimizers of $\lmax$ at their density.  And the non-monotone
trajectory data suggests that spectral concentration peaks
well before $k = \sqrt{N}$ and then relaxes, though the
mechanism governing the relaxation rate is unknown.

\begin{conjecture}[Spectral budget squeeze]
\label{conj:squeeze}
For $k > \sqrt{N} + \omega(N^{1/4})$, the Parseval and
fourth-moment constraints on the power spectrum of a Sidon set
$A \subseteq \Z_N$ with $|A| = k$ force
$\lmax(A) < k - 1$.  Combined with the pigeonhole lower
bound $\lmax \ge k(N-k)/(N-1)$ (which requires
$k(k-1) \le N-1$, i.e., $k \le \sqrt{N} + O(1)$),
this would give $|A| \le \sqrt{N} + O(N^{1/4})$.
\end{conjecture}

\noindent\textbf{Open gap.} The missing ingredient is a
quantitative upper bound on $\lmax(A)$ for Sidon sets at
densities above $k = \sqrt{N}$.  The Ortega-Cerd\`a--Prendiville
theorem gives $\lmax = o(N)$ for extremal sets, but no explicit
bound as a function of~$k$ alone.  The non-monotone trajectory
(\cref{obs:trajectory}) provides computational evidence that
$\lmax/k$ decreases past the peak, but the rate of decrease is
not determined.

%% ────────────────────────────────────────────────
%% [C8] New §5.4: MDL / H² / Gödel connection.
%%   Motivational only, mathematically inert.  Two citations
%%   to Mendoza2026a, Mendoza2026b.
%% ────────────────────────────────────────────────
\subsection{Broader context: information-theoretic parsimony}

\begin{remark}[Non-mathematical motivation]
The $S_2$ blindness barrier is an instance of a broader
pattern: every self-consistent dynamical description
requires a minimum closure term that is blind to fine local
structure.  In the language of minimum description length
(MDL) theory, the Sidon condition fixes the model complexity
$L_M$ (via $E_2 = 2k^2 - k$), and the blindness barrier says
that the residual data cost $L_{D|M}$ cannot be reduced by
any $E_2$-determined statistic.  Only statistics that see the
full spectrum --- above the $S_2$ floor --- can further compress.

This pattern appears independently in the author's analysis of
G\"odel's incompleteness as an information-processing
constraint~\cite{Mendoza2026a} and in the broader H$^2$
research program on phase-transition detection across
domains~\cite{Mendoza2026b}.  In both settings, a universal
second-moment floor (Shannon, $S_2$, additive energy) coexists
with a discriminating first-moment statistic (von Neumann,
$S_1$, spectral peak) that sees geometric structure invisible
to the floor.

We emphasize that this remark is motivational:
\cref{thm:S2-universal,thm:flatness} stand on their own
algebraic proofs without reference to MDL, and the connection
to G\"odel or phase transitions is an analogy, not a theorem.
\end{remark}

%% ────────────────────────────────────────────────
%% [C9] Open questions: dedicated §6 with 4 numbered
%%   \begin{question} items.
%% ────────────────────────────────────────────────
\section{Open questions}\label{sec:open}

\begin{question}\label{q:peak-location}
At fixed~$N$, does the spectral peak location $k^* \approx 0.3\sqrt{N}$
scale precisely as $c\sqrt{N}$ for a universal constant~$c$?
Is the constant $c \approx 0.3$ an artifact of the greedy construction,
or does it persist for random or optimized Sidon sets?
\end{question}
...
\begin{question}\label{q:spectral-gap-growth}
The spectral gap between Singer sets and random Sidon sets
at matched~$k$ grows with~$q$ (\cref{tab:singer-anomaly}).
What is the asymptotic growth rate?  Is the ratio
$\lmax(\text{random})/\lmax(\text{Singer})$ bounded or
unbounded as $q \to \infty$?
\end{question}

\begin{question}\label{q:higher-renyi}
For R\'enyi entropies $S_\alpha$ with $\alpha \ge 3$, the
relevant energy $E_\alpha(A) = \sum_n r_\alpha(n)^\alpha$
is \emph{not} universal for Sidon sets (unlike $E_2$).
Can $S_3$ or higher-order entropies provide additional
discrimination, and does this yield constraints relevant
to Erd\H{o}s Problem~\#30?
\end{question}
...
\bibitem{Mendoza2026a}
K.~Mendoza.
\newblock {G\"odel's incompleteness as an information-processing constraint:
  a minimum description length perspective}.
\newblock Preprint, April 2026.

\bibitem{Mendoza2026b}
K.~Mendoza.
\newblock {The H$^2$ research program: phase-transition detection via
  information-geometric invariants}.
\newblock Technical report, MendozaLab, 2026.
